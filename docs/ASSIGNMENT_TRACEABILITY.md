...
| ASG-DATA-023 | §2.3 p8 | On Kubernetes, deleting the Postgres pod preserves every row | Evidence | Artfever | #54 | `k8s/base/postgres.yaml` | DEMO | `docs/evidence/k8s-pg-persistence.txt` | Measured: 30 rows and full-row fingerprint preserved; review pending |
...
| ASG-K8S-012 | §3.3 p15 | HorizontalPodAutoscaler (autoscaling/v2) on the backend | Mandatory | Artfever | #50 | `k8s/base/hpa.yaml` | CI | `docs/evidence/k8s-load-README.md` | Implemented; measured locally; partner review pending |
| ASG-K8S-013 | §3.3 p15 | HPA: `minReplicas: 2`, `maxReplicas: 10`, CPU Utilization target 60% | Mandatory | Artfever | #50 | `k8s/base/hpa.yaml` | INS | `docs/evidence/k8s-load-README.md` | Implemented; measured locally; partner review pending |
| ASG-K8S-014 | §3.3 p15 | HPA behavior: scaleDown `stabilizationWindowSeconds: 300`; scaleUp `stabilizationWindowSeconds: 0` | Mandatory | Artfever | #50 | `k8s/base/hpa.yaml` | INS | `docs/evidence/k8s-load-README.md` | Implemented; measured locally; partner review pending |
...
| ASG-K8S-023 | §3.3 p16 | metrics-server installed | Mandatory | Artfever | #50 | docs / overlay | DEMO | `docs/evidence/k8s-load-README.md` | Implemented; measured locally; partner review pending |
| ASG-K8S-024 | §3.3 p16 | Generate load with k6 or hey and capture the scale-out | Evidence | Artfever | #50 | `load/k6-script.js` | DEMO, MEAS | `docs/evidence/k8s-load-README.md` | Implemented; measured locally; partner review pending |
| ASG-K8S-025 | §3.3 p16; §5.8 p26 | Commit `kubectl get hpa -w` output showing replicas rising | Evidence | Artfever | #50 | `docs/evidence/` | DOC | `docs/evidence/k8s-load-README.md` | Implemented; measured locally; partner review pending |
| ASG-K8S-026 | §3.3 p16; §5.8 p26 | Commit a chart of replicas against offered load over time | Evidence | Artfever | #50 | `docs/evidence/` | DOC | `docs/evidence/k8s-load-README.md` | Implemented; measured locally; partner review pending |
| ASG-K8S-027 | §3.3 p16 | 3–5 sentences on the lag between load arriving and capacity arriving | Evidence | Artfever | #50 | `docs/evidence/k8s-load-README.md` (engineering notes integration: #54) | DOC | `docs/evidence/k8s-load-README.md` | Implemented; measured locally; partner review pending |
| ASG-K8S-028 | §3.3 p16 | Vertical Pod Autoscaler installed and run on the backend in recommender mode (`updateMode: "Off"`) | Mandatory | Artfever | #50 | `k8s/base/vpa.yaml` | INS, DEMO | `docs/evidence/k8s-load-README.md` | Implemented; measured locally; partner review pending |
| ASG-K8S-029 | §3.3 p16 | VPA loop: record guessed requests → run load test → commit Target / Lower Bound / Upper Bound from `kubectl describe vpa backend-vpa` → update requests → re-run the load test and report the HPA change | Evidence | Artfever | #50 | `docs/evidence/` | DOC | `docs/evidence/k8s-load-README.md` | Implemented; measured locally; partner review pending |
| ASG-K8S-030 | §3.3 p16–17 | Explain why VPA runs in Off mode: the HPA/VPA feedback conflict when both act on CPU | Evidence | Artfever | #50 | `docs/evidence/k8s-load-README.md` (engineering notes integration: #54) | DOC | `docs/evidence/k8s-load-README.md` | Implemented; measured locally; partner review pending |
...
| ASG-CICD-002 | §3.4 p17 | Two branches: `dev` for work, `main` for deployable software; main protected with required checks and one approval | Mandatory | TahaSohail-Goat | #51 | repo settings | DEMO | screenshot | Done: rulesets active with PR and required checks (`docs/evidence/ruleset-*.json`); screenshot pending |
...
| ASG-CICD-012 | §3.4 p18 | `cd.yml` runs on push to main | Mandatory | Artfever | #52 | `cd.yml` | CI | `docs/evidence/cd-local-rollback.txt`; Actions run pending | Implemented; local checks/rehearsal; main CD and video evidence pending |
| ASG-CICD-013 | §3.4 p18 | `test`: the full suite again on the merged result | Mandatory | Artfever | #52 | `cd.yml` | CI | `docs/evidence/cd-local-rollback.txt`; Actions run pending | Implemented; local checks/rehearsal; main CD and video evidence pending |
| ASG-CICD-014 | §3.4 p18 | `build-push` (`needs: test`): build both images, push to GHCR tagged `${{ github.sha }}` and `latest` | Mandatory | Artfever | #52 | `cd.yml` | CI | `docs/evidence/cd-local-rollback.txt`; Actions run pending | Implemented; local checks/rehearsal; main CD and video evidence pending |
| ASG-CICD-015 | §3.4 p18 | Emit an SBOM with Syft | Mandatory | Artfever | #52 | `cd.yml` | CI | `docs/evidence/cd-local-rollback.txt`; Actions run pending | Implemented; local checks/rehearsal; main CD and video evidence pending |
| ASG-CICD-016 | §3.4 p18 | Capture the image digest as a job output | Mandatory | Artfever | #52 | `cd.yml` | CI | `docs/evidence/cd-local-rollback.txt`; Actions run pending | Implemented; local checks/rehearsal; main CD and video evidence pending |
| ASG-CICD-017 | §3.4 p18 | `deploy-k8s` (`needs: build-push`): spin up a kind/k3d cluster in the runner and apply `overlays/prod` with the SHA tag | Mandatory | Artfever | #52 | `cd.yml` | CI | `docs/evidence/cd-local-rollback.txt`; Actions run pending | Implemented; local checks/rehearsal; main CD and video evidence pending |
| ASG-CICD-018 | §3.4 p18 | Wait for `kubectl rollout status` | Mandatory | Artfever | #52 | `cd.yml` | CI | `docs/evidence/cd-local-rollback.txt`; Actions run pending | Implemented; local checks/rehearsal; main CD and video evidence pending |
| ASG-CICD-019 | §3.4 p18 | Run a smoke test against the Ingress | Mandatory | Artfever | #52 | `cd.yml` | CI | `docs/evidence/cd-local-rollback.txt`; Actions run pending | Implemented; local checks/rehearsal; main CD and video evidence pending |
| ASG-CICD-020 | §3.4 p18 | Print `kubectl get hpa` | Mandatory | Artfever | #52 | `cd.yml` | CI | `docs/evidence/cd-local-rollback.txt`; Actions run pending | Implemented; local checks/rehearsal; main CD and video evidence pending |
| ASG-CICD-021 | §3.4 p18 | `release.yml` on tag `v*`: build, push semver tags, generate release notes | Mandatory | Artfever | #52 | `release.yml` | CI | `docs/evidence/cd-local-rollback.txt`; Actions run pending | Implemented; local checks/rehearsal; main CD and video evidence pending |
| ASG-CICD-022 | §3.4 p18 | `needs:` on every publishing and deploying job | Mandatory | Artfever | #52 | all workflows | INS | `docs/evidence/cd-local-rollback.txt`; Actions run pending | Implemented; local checks/rehearsal; main CD and video evidence pending |
| ASG-CICD-023 | §3.4 p18 | Deploy by immutable reference (commit SHA; digest for bonus); `:latest` may be pushed but never deployed; "what is production running?" has a one-word answer usable in `git show` | Mandatory | Artfever | #52 | `cd.yml`, `k8s/overlays/prod` | INS | `docs/evidence/cd-local-rollback.txt`; Actions run pending | Implemented; local checks/rehearsal; main CD and video evidence pending |
...
| ASG-CICD-028 | §3.4 p18 | Rollback mechanism 1: `kubectl rollout undo deployment/backend -n civicpulse` | Mandatory | Artfever | #52 | `docs/RUNBOOK.md` | DEMO | `docs/evidence/cd-local-rollback.txt`; Actions run pending | Implemented; local checks/rehearsal; main CD and video evidence pending |
| ASG-CICD-029 | §3.4 p18–19 | Rollback mechanism 2: re-apply the previous overlay with the previous SHA | Mandatory | Artfever | #52 | `docs/RUNBOOK.md` | DEMO | `docs/evidence/cd-local-rollback.txt`; Actions run pending | Implemented; local checks/rehearsal; main CD and video evidence pending |
| ASG-CICD-030 | §3.4 p19 | Both rollbacks demonstrated on video, with an explanation of when to use each | Evidence | Artfever | #52 | `docs/RUNBOOK.md` | DEMO | `docs/evidence/cd-local-rollback.txt`; Actions run pending | Implemented; local checks/rehearsal; main CD and video evidence pending |
| ASG-CICD-031 | §1.4 p3 | A bad deploy can be undone in thirty seconds | Mandatory | Artfever | #52 | `docs/RUNBOOK.md` | MEAS | `docs/evidence/cd-local-rollback.txt`; Actions run pending | Implemented; local checks/rehearsal; main CD and video evidence pending |
...
| ASG-DOC-016 | §4 J p22; §5.2 p23 | `docs/ENGINEERING-NOTES.md` answers all eight questions with references to the team's own files and lines (generic answers score zero) | Mandatory | Artfever | #54 | `docs/ENGINEERING-NOTES.md` | DOC | `docs/ENGINEERING-NOTES.md` | Partial: real CD failure documented; >1-hour duration unverified |
...
| ASG-DOC-019 | §5.2 p23 | EN-Q3: the exact line guaranteeing build-once-deploy-many and what breaks without it | Evidence | Artfever | #54 | notes | DOC | `docs/ENGINEERING-NOTES.md` | Written with source/line references; review pending |
| ASG-DOC-020 | §5.2 p23 | EN-Q4: what "correct" means for a probabilistic LLM component and how CI stays deterministic (Lecture 01, slide 34) | Evidence | Artfever | #54 | notes | DOC | `docs/ENGINEERING-NOTES.md` | Written with source/line references; review pending |
| ASG-DOC-021 | §5.2 p23 | EN-Q5: measured HPA lag in seconds; where the time went; what would reduce it | Evidence | Artfever | #54 | notes | MEAS | `docs/ENGINEERING-NOTES.md` | Written with source/line references; review pending |
| ASG-DOC-022 | §5.2 p24 | EN-Q6: why VPA is Off; failure mode of running it in Auto alongside HPA | Evidence | Artfever | #54 | notes | DOC | `docs/ENGINEERING-NOTES.md` | Written with source/line references; review pending |
| ASG-DOC-023 | §5.2 p24 | EN-Q7: where the hosted-LLM caller lives given `internal: true`, and how it was resolved | Evidence | Artfever | #54 | notes | DOC | `docs/ENGINEERING-NOTES.md` | Written with source/line references; review pending |
| ASG-DOC-024 | §5.2 p24 | EN-Q8: a failure that cost > 1 hour — symptoms, wrong first belief, the exact command/log line that revealed the truth | Evidence | Artfever | #54 | notes | DOC | `docs/ENGINEERING-NOTES.md` | Partial: real CD failure documented; >1-hour duration unverified |
