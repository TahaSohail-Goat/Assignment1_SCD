...
| ASG-GEN-010 | §1.4 p3 | Every claim in the README can be demonstrated | Mandatory | Artfever | #55 | `README.md` | INS | `README.md`, `backend/tests/test_readme.py`, `docs/evidence/screenshots-README.md`; linked source/evidence | Implemented; README has a 'Demo video and remaining handover' section and `test_readme.py` checks every link |
...
| ASG-FR-008 | §2.1 p4; §4 B p19 | Dashboard: paginated, filterable list (category, priority, status) | Mandatory | Artfever | #35, #102 | `frontend/src/pages/` | UT | `frontend/src/pages/Dashboard.tsx`, `frontend/tests/views.test.tsx` | Implemented (#35, #36); #102 adds tested stale-row clearing, retry and last-page recovery (merged in PR #103; awaiting promotion) |
...
| ASG-FR-036 | §4 C p19 | All endpoints in the API contract are implemented to contract with correct status codes (rubric says "ten"; the table lists nine) | Mandatory | TahaSohail-Goat | #39 | `backend/app/routes/` | IT | `backend/tests/test_contract.py` | All nine endpoints implemented; #46 merged. Contract and metadata tests pass; see backend/tests/test_contract.py and test_meta.py |
...
| ASG-NFR-014 | §3.4 p17; §4 B p19 | ≥ 5 meaningful frontend component tests (Vitest) passing in CI | Mandatory | Artfever | #36, #102 | `frontend/tests/` | CI | `frontend/tests/views.test.tsx` (13 passing locally) | Implemented (#36); #102 adds two reproduced Dashboard regressions; merged/reviewed in #103 and #108; all 21 frontend tests pass in CI 36243422641 |
...
| ASG-AI-016 | §2.5 p11 | Cache triage results by content hash in Redis, 24 h TTL | Mandatory | Artfever | #46 | `backend/app/services/` | IT | `backend/tests/test_injection_cache.py` | Verified against real Redis 7.4.11 in docs/evidence/final-audit-cache.json (2 requests, 1 inference, TTL 86400 seconds) |
| ASG-AI-017 | §2.5 p11 | Report the measured hit rate of the content-hash cache | Evidence | Artfever | #46 | `docs/ENGINEERING-NOTES.md` | MEAS | `docs/TRIAGE.md` (1 hit / 2 requests, in-memory) | Real Redis service-level interval measured in final-audit-cache.json: 1 hit / 2 requests = 50%; not a representative production workload |
...
| ASG-K8S-002 | §3.3 p14 | Manifests organised with Kustomize (`base/` + `overlays/dev` + `overlays/prod`); Helm acceptable with an ADR | Mandatory | TahaSohail-Goat | #49 | `k8s/` | CI | `backend/tests/test_k8s_manifests.py` | Implemented (P09-S01, #49); rendered/schema-checked in CI and deployed by main CD 36235766515; prior local captures retained |
| ASG-K8S-003 | §3.3 p14 | Everything in namespace `civicpulse`, never `default` | Mandatory | TahaSohail-Goat | #49 | `k8s/base/namespace.yaml` | INS, CI | `backend/tests/test_k8s_manifests.py` | Implemented (P09-S01, #49); rendered/schema-checked in CI and deployed by main CD 36235766515; prior local captures retained |
| ASG-K8S-004 | §3.3 p14 | `backend` Deployment with ≥ 2 replicas | Mandatory | TahaSohail-Goat | #49 | `k8s/base/backend.yaml` | CI | `backend/tests/test_k8s_manifests.py` | Implemented (P09-S01, #49); rendered/schema-checked in CI and deployed by main CD 36235766515; prior local captures retained |
| ASG-K8S-005 | §3.3 p14 | `frontend` Deployment with ≥ 2 replicas | Mandatory | TahaSohail-Goat | #49 | `k8s/base/frontend.yaml` | CI | `backend/tests/test_k8s_manifests.py` | Implemented (P09-S01, #49); rendered/schema-checked in CI and deployed by main CD 36235766515; prior local captures retained |
| ASG-K8S-006 | §3.3 p14 | `postgres` is a StatefulSet with `volumeClaimTemplates` → PVC; be ready to explain why a Deployment is wrong | Mandatory | TahaSohail-Goat | #49 | `k8s/base/postgres.yaml` | CI, DOC | `backend/tests/test_k8s_manifests.py` | Implemented (P09-S01, #49); rendered/schema-checked in CI and deployed by main CD 36235766515; prior local captures retained |
| ASG-K8S-007 | §3.3 p14 | `redis` is a Deployment with a PVC | Mandatory | TahaSohail-Goat | #49 | `k8s/base/redis.yaml` | CI | `backend/tests/test_k8s_manifests.py` | Implemented (P09-S01, #49); rendered/schema-checked in CI and deployed by main CD 36235766515; prior local captures retained |
| ASG-K8S-008 | §3.3 p14 | Four Services, all ClusterIP; the database is never NodePort or LoadBalancer | Mandatory | TahaSohail-Goat | #49 | `k8s/base/` | CI | `backend/tests/test_k8s_manifests.py` | Implemented (P09-S01, #49); rendered/schema-checked in CI and deployed by main CD 36235766515; prior local captures retained |
...
| ASG-K8S-010 | §3.3 p14 | ConfigMap holds non-secret configuration | Mandatory | TahaSohail-Goat | #49 | `k8s/base/configmap.yaml` | INS | `backend/tests/test_k8s_manifests.py` | Implemented (P09-S01, #49); rendered/schema-checked in CI and deployed by main CD 36235766515; prior local captures retained |
| ASG-K8S-011 | §3.3 p14 | Secret for DB password and LLM API key; committed manifests contain placeholders only | Mandatory | TahaSohail-Goat | #49 | `k8s/base/secret.yaml` | INS, CI | secret scan, `backend/tests/test_k8s_manifests.py` | Implemented (P09-S01, #49); rendered/schema-checked in CI and deployed by main CD 36235766515; prior local captures retained |
...
| ASG-K8S-015 | §3.3 p14 | PodDisruptionBudget `minAvailable: 1` on the backend | Mandatory | TahaSohail-Goat | #49 | `k8s/base/pdb.yaml` | INS | `backend/tests/test_k8s_manifests.py` | Implemented (P09-S01, #49); rendered/schema-checked in CI and deployed by main CD 36235766515; prior local captures retained |
| ASG-K8S-016 | §3.3 p15 | `startupProbe`: httpGet `/health` port 8000, `failureThreshold: 30`, `periodSeconds: 2` | Mandatory | TahaSohail-Goat | #49 | `k8s/base/backend.yaml` | INS | `backend/tests/test_k8s_manifests.py` | Implemented (P09-S01, #49); rendered/schema-checked in CI and deployed by main CD 36235766515; prior local captures retained |
| ASG-K8S-017 | §3.3 p15 | `livenessProbe`: httpGet `/health`; must NOT depend on the database | Mandatory | TahaSohail-Goat | #49 | `k8s/base/backend.yaml` | INS | `backend/tests/test_k8s_manifests.py` | Implemented (P09-S01, #49); rendered/schema-checked in CI and deployed by main CD 36235766515; prior local captures retained |
| ASG-K8S-018 | §3.3 p15 | `readinessProbe`: httpGet `/ready`; SHOULD depend on the database | Mandatory | TahaSohail-Goat | #49 | `k8s/base/backend.yaml` | INS | `backend/tests/test_k8s_manifests.py` | Implemented (P09-S01, #49); rendered/schema-checked in CI and deployed by main CD 36235766515; prior local captures retained |
| ASG-K8S-019 | §3.3 p15 | Rolling update `maxSurge: 1`, `maxUnavailable: 0` | Mandatory | TahaSohail-Goat | #49 | `k8s/base/backend.yaml` | INS | `backend/tests/test_k8s_manifests.py` | Implemented (P09-S01, #49); rendered/schema-checked in CI and deployed by main CD 36235766515; prior local captures retained |
| ASG-K8S-020 | §3.3 p15 | `terminationGracePeriodSeconds` and a `preStop` sleep so the pod leaves Service endpoints before it stops accepting connections | Mandatory | TahaSohail-Goat | #49 | `k8s/base/backend.yaml` | INS | `backend/tests/test_k8s_manifests.py` | Implemented (P09-S01, #49); rendered/schema-checked in CI and deployed by main CD 36235766515; prior local captures retained |
...
| ASG-K8S-022 | §3.3 p16; §4 H p21 | `resources.requests` (incl. `cpu`) and `limits` set on every container — the HPA needs the request as denominator | Mandatory | TahaSohail-Goat | #49 | `k8s/base/` | INS, CI | `backend/tests/test_k8s_manifests.py` | Implemented (P09-S01, #49); rendered/schema-checked in CI and deployed by main CD 36235766515; prior local captures retained |
...
| ASG-GH-006 | §4 A p19 | ≥ 35 commits | Mandatory | TahaSohail-Goat | #56 | git history | MEAS | `git shortlog -sn` | Done: 179 commits on origin/dev at 39fca0c by git shortlog -sn, threshold 35 |
...
| ASG-GH-008 | §4 A p19 | Neither partner below 35% by `git shortlog -sn` | Mandatory | TahaSohail-Goat | #56 | git history | MEAS | `git shortlog -sn` | PASS at dev 39fca0c: git shortlog -sn origin/dev gives Taha 109 / Artfever 70 (39.1%), threshold 35%. Refresh after final promotion |
...
The layout root is named `civicpulse/` in the source; the product may be renamed (ASG-GEN-004) so the repository root name is not significant. Directory placeholders (`.gitkeep`) were created in Phase 00; the current implemented inventory is shown in [`REPOSITORY_STRUCTURE.md`](REPOSITORY_STRUCTURE.md).
...
## Audit follow-up #107 (2026-09-26)

Artfever placed the video (ASG-DOC-014/015, ASG-SUB-004) and live provider comparison (ASG-AI-025) on hold until the end. The separate ruleset/instructor screenshot and exact-deadline tasks are removed by owner direction, not claimed as captured. Unicode validation ASG-FR-005 is corrected and boundary-tested in #107. Final verification status is in [FINAL_SUBMISSION_CHECKLIST.md](FINAL_SUBMISSION_CHECKLIST.md); snapshot counts above deliberately identify their revision.

## Verified audit snapshot #109

Dev `39fca0c`: 363 backend tests, 94.42% coverage, 21 frontend tests, all ten CI jobs pass (run 36243422641). [Final audit index](evidence/final-audit-index.md) records GitHub evidence, real Redis cache interval, compiled-asset scan and remaining historical commit-prefix exceptions. The final checklist controls sign-off; prior implementation statuses above are not a substitute for it.
