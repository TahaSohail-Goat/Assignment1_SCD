...
1. [ ] every acceptance box of the parent and sub-issues is ticked; the issues still open stay open until the integration PR merges (its `Closes` line closes them), and the ones closed earlier stay as they are
...
6. [ ] the **integration PR** `dev` → `main` is opened (its `Closes #a, #b, …` line names exactly the open issues of the phase, see condition 1 of section 2), approved by the other contributor on its final head, and merged with a **merge commit** (prompt 7.7, second and third blocks); nothing else is merged into `dev` while it is open, because a push to `dev` dismisses the approval
7. [ ] verified on `main`: `git fetch origin --prune`, then `git show origin/main:docs/PHASE_STATUS.md` shows the phase row Complete, and `gh issue list --label <phase-label> --state open --json number --jq '.[].number'` prints nothing (no issue of the phase is open)
8. [ ] only now may the next phase's parent issue be started

## 7. Prompt library

Paste one block as the first message of a step. Replace the placeholders: `<ME>` = your GitHub user (`TahaSohail-Goat` or `Artfever`), `<OTHER>` = the other one, `<N>` = issue or PR number, `<PHASE>` = phase number, `<parent>` = the phase's parent issue number, `<phase-label>` = the phase's label (for example `phase:00-foundation`), `<TRAILER>` = your trailer from section 1.
