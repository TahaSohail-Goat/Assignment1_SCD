<!--
  FAITHFUL MARKDOWN TRANSCRIPTION of docx/ASSIGNMENT_SOURCE.pdf (26 pages, unchanged).
  - Source wording is reproduced verbatim. Nothing has been summarised, reordered or corrected.
  - HTML comments of the form "PDF page N" mark where each source page begins.
  - Text inside "> [Transcriber note: ...]" blocks is NOT part of the assignment; it flags a
    source anomaly. All such notes are catalogued in docx/EXTRACTION_NOTES.md.
  - Emphasis (bold/italic) was reproduced on a best-effort basis from the rendered pages.
  - This file is the authoritative searchable copy. If it ever disagrees with the PDF, the PDF wins.
-->

# Assignment 01 — CivicPulse (transcription)
...
> [Transcriber note: in the PDF the first two rows span both columns.]
...
![CivicPulse system architecture diagram (PDF page 3)](assets/architecture-diagram.png)

> [Transcriber note: the diagram is an embedded image in the PDF; it is extracted unchanged to `docx/assets/architecture-diagram.png`. Its content is transcribed below as text/Mermaid for searchability. Labels are reproduced exactly as drawn.]

```mermaid
flowchart TD
    C(["Citizen / Operator"]) -- "HTTP" --> F
    subgraph EDGE ["docker network: edge"]
        F["frontend · React + Vite → nginx<br/>multi-stage image"]
    ...
    B --> PG
    B --> RD
    B --> TP{{"TriageProvider (interface)"}}
    TP -- "default" --> G["Groq or Gemini · free tier · JSON mode"]
    TP -- "CI" --> S["SimulatedTriage · deterministic fake"]
    G -. "timeout · 429 · bad JSON" .-> R["RuleBasedTriage · fallback"]
```
...
> [Transcriber note: the API contract table is split across PDF pages 5 and 6 (header repeated in this transcription only for readability; in the PDF page 6 has no header row). The `/api/stats` behaviour cell ends exactly at "\`X-Cache: HIT" in the source (an opening backtick that is never closed) — the remainder appears to have been lost when the original document was produced, most likely because a `|` inside a Markdown table cell was read as a column delimiter. §2.4 states `X-Cache: HIT|MISS`. The cell is transcribed as it appears; the backtick is escaped so it displays literally. See EXTRACTION_NOTES.md EN-04.]

/health and /ready are separate because Kubernetes uses them for different decisions: a failing liveness probe **restarts your pod**, a failing readiness probe **removes it from the Service**. Wire them backwards and a slow database becomes a restart loop across your entire deployment. This is Lecture 04's depends_on lesson, promoted to production consequences.
...
**Structured logging.** JSON to stdout — never to a file, because a container's filesystem is ephemeral and your log shipper reads stdout. Every log line carries a request_id propagated from an X-Request-ID header (generate one if absent). One WARNING per triage fallback with the complaint id, the provider and the error class.
...
> [Transcriber note: the minimum-schema table is split across PDF pages 7 and 8; it is one table.]
...
> [Transcriber note: the `<cite index="…">` / `</cite>` tags are literal visible text in the source PDF (markup residue from the document's authoring tool). They are preserved here inside code spans so they remain visible. They carry no requirement. See EXTRACTION_NOTES.md EN-03.]
...
4. **Fall back to RuleBasedTriage.** Record triaged_by = "rules:fallback". A user must never see a 500 because a third party was rate-limited.
...
With TRIAGE_PROVIDER=llm the same input can produce different output. Your test suite must still be green on every single run — a flaky pipeline trains a team to ignore red, which is worse than having no pipeline.
...
**Frontend** — node:22-alpine builds, nginx:1.27-alpine serves. The final image contains no Node, no node_modules, no source. Report both stage sizes; a frontend image over ~60 MB means the multi-stage split is not doing its job.

**.dockerignore in each build context** — .git, node_modules, .venv, \_\_pycache\_\_, .env, test fixtures. Report build-context size before and after, with numbers.
...
> [Transcriber note: this one YAML block (`networks:` with `edge` and `internal`) is split across PDF pages 12 and 13; the two fragments above form a single block: `networks:` / `edge:` / `internal:` at the same indentation.]
...
**Required Compose engineering:** healthchecks on every service; depends_on: condition: service_healthy; all credentials via ${...} from .env; .env.example committed; .env gitignored; every image tag pinned; restart: unless-stopped; resource limits under deploy.resources; no published port on database or cache in the production file.

<!-- PDF page 14 -->

Two files: compose.yaml (dev, build:) and compose.prod.yaml (deploy, image: with ${IMAGE_TAG}, no build: key anywhere).
...
- All credentials from GitHub Secrets. A **scoped, revocable registry token**, never an account password. GITHUB_TOKEN with packages: write is the cleanest path for GHCR.
...
- Teams of 3 with the frontend owned by one member. Raise the PR requirement to 7 and the commit floor to 30% each.
...
> [Transcriber note: the source numbers this section 5.7; there is no section 5.6. Numbering is preserved as in the source. The tree is split across PDF pages 25 and 26: page 26 begins at the `overlays/{dev,prod}/kustomization.yaml` line. See EXTRACTION_NOTES.md EN-07.]
